...
function EFFECT:Init(data)
    ...
    timer.Simple(Delay, function()
        if IsValid(Entity) then
            ...
            if snd2 == "" then snd2 = "simulated_vehicles/sfx/ex_backfire_" .. math.Round(math.random(1, 4), 1) .. ".ogg" end
            local snd = bdamaged and snd1 or snd2
            Entity.CurBackFireSound = CreateSound(Entity, snd)
            Entity.CurBackFireSound:Play()
            Entity:CallOnRemove("stopbfsounds", function(Entity) if Entity.CurBackFireSound then Entity.CurBackFireSound:Stop() end end)
            timer.Simple(5, function()
                if not IsValid(Entity) then return end
                if Entity.CurBackFireSound then Entity.CurBackFireSound:Stop() end
            end)
