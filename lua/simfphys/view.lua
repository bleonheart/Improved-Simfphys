...
hook.Add("StartCommand", "simfphys_lockview", function(ply, ucmd)
    ...
    local dir = 0
    if vehicle.lockedyaw < 90 and vehicle.lockedyaw > -90 then dir = math.abs(vehicle.lockedyaw - 90) end
    if vehicle.lockedyaw >= 90 then dir = -math.abs(vehicle.lockedyaw - 90) end
    if vehicle.lockedyaw < -90 and vehicle.lockedyaw >= -270 then dir = -math.abs(vehicle.lockedyaw + 270) end
    vehicle.lockedyaw = vehicle.lockedyaw + dir * 0.05
    vehicle.lockedpitch = vehicle.lockedpitch + (LockedPitch - vehicle.lockedpitch) * 0.05
    if ply:GetInfoNum("cl_simfphys_ms_lockpitch", 0) == 1 then ang.p = vehicle.lockedpitch end
