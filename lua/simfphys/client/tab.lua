...
local k_list = {{k_fwd, KEY_W, "Forward"}, {k_bck, KEY_S, "Reverse"}, {k_left, KEY_A, "Steer Left"}, {k_right, KEY_D, "Steer Right"}, {k_wot, KEY_LSHIFT, "Throttle Modifier"}, {k_clth, KEY_LALT, "Clutch"}, {k_gup, MOUSE_LEFT, "Gear Up"}, {k_gdn, MOUSE_RIGHT, "Gear Down"}, {k_hbrk, KEY_SPACE, "Handbrake"}, {k_cc, KEY_R, "Cruise Control"}, {k_lgts, KEY_F, "Lights"}, {k_flgts, KEY_V, "Foglights"}, {k_horn, KEY_H, "Horn / Siren"}, {k_engine, KEY_I, "Start/Stop Engine"}, {k_afwd, KEY_PAD_8, "Tilt Backward"}, {k_abck, KEY_PAD_2, "Tilt Forward"}, {k_aleft, KEY_A, "Tilt Left"}, {k_aright, KEY_D, "Tilt Right"}, {k_lock, KEY_NONE, "Lock / Unlock"}, {k_turnmenu, KEY_COMMA, "Turnsignals"},}
local function simplebinder(x, y, tbl, num, parent)
    ...
    TextLabel.Paint = function() draw.SimpleText(kentry[3], "DSimfphysFont", 175 * 0.5, sizey * 0.5, Color(100, 100, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) end
    return binder
end
...
local function simplebinder_old(x, y, tbl, num, parent, sizex, sizey)
    ...
    TextLabel.Paint = function() draw.SimpleText(kentry[3], "DSimfphysFont", sizex * 0.25, 20, Color(100, 100, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) end
    return binder
end
...
local function buildcontrolsmenu(self)
    ...
    TextLabel.Paint = function() draw.SimpleTextOutlined("You need to re-enter the vehicle in order for the changes to take effect!", "DSimfphysFont_hint", 300, 20, Color(255, 0, 0, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 255)) end
    local yy = 45
    local binders = {}
...
local function buildserversettingsmenu(self)
    ...
    if LocalPlayer():IsSuperAdmin() then
        ...
        for k, v in pairs(simfphys.TractionData) do
            ...
            DemSliders[k].OnValueChanged = function(item, value) NewTractionData[k] = value end
            y = y + 25
        end
...
hook.Add("PopulateVehicles", "!!!add_simfphys_to_vehicles", function(pnlContent, ogtree, node)
    ...
    if istable(jcon) and file.Exists("lua/bin/gmcl_joystick_win32.dll", "GAME") then
        ...
        node.DoClick = function(self) RunConsoleCommand("joyconfig") end
    end
