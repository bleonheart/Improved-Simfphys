...
local function PrecacheArc(cx, cy, radius, thickness, startang, endang, roughness, bClockwise)
    ...
    if bClockwise then step = math.abs(roughness) * -1 end
    local inner = {}
...
local function drawsimfphysHUD(vehicle, SeatCount)
    ...
    if AltHud and not ForceSimpleHud and not SimpleHudIsForced then
        ...
        if Hudmpg then calc_fueluse = 235.214 / calc_fueluse end
    ...
    if cruisecontrol then draw.SimpleText("cruise", "simfphysfont", s_xpos + sizex * 0.115 + o_x, s_ypos + sizey * 0.035 + o_y, color_cruisecontrol_orange, 2, 1) end
...
local function drawTurnMenu(vehicle)
    if input.IsKeyDown(cl_simfphys_keyforward:GetInt()) or input.IsKeyDown(cl_simfphys_key_air_forward:GetInt()) then turnmode = 0 end
    if input.IsKeyDown(cl_simfphys_keyleft:GetInt()) or input.IsKeyDown(cl_simfphys_key_air_left:GetInt()) then turnmode = 2 end
    if input.IsKeyDown(cl_simfphys_keyright:GetInt()) or input.IsKeyDown(cl_simfphys_key_air_right:GetInt()) then turnmode = 3 end
    if input.IsKeyDown(cl_simfphys_keyreverse:GetInt()) or input.IsKeyDown(cl_simfphys_key_air_reverse:GetInt()) then turnmode = 1 end
    local cX = ScrW() / 2
    local cY = ScrH() / 2
    ...
    if turnmode == 0 then surface.SetDrawColor(240, 200, 0, 255) end
    surface.DrawLine(cX - sx * 0.3, cY - sy - sy * 0.3, cX + sx * 0.3, cY - sy + sy * 0.3)
    surface.DrawLine(cX + sx * 0.3, cY - sy - sy * 0.3, cX - sx * 0.3, cY - sy + sy * 0.3)
    surface.SetDrawColor(240, 200, 0, 100)
    if turnmode == 2 then surface.SetDrawColor(240, 200, 0, 255) end
    ...
    surface.DrawLine(cX - sx, cY - sy * 0.3, cX - sx - sx * 0.3, cY)
    surface.SetDrawColor(240, 200, 0, 100)
    if turnmode == 3 then surface.SetDrawColor(240, 200, 0, 255) end
    surface.DrawLine(cX + sx - sx * 0.3, cY - sy * 0.15, cX + sx - sx * 0.3, cY + sy * 0.15)
    surface.DrawLine(cX + sx - sx * 0.3, cY + sy * 0.15, cX + sx, cY + sy * 0.15)
    surface.DrawLine(cX + sx - sx * 0.3, cY - sy * 0.15, cX + sx, cY - sy * 0.15)
    surface.DrawLine(cX + sx, cY - sy * 0.3, cX + sx, cY - sy * 0.15)
    ...
    surface.DrawLine(cX + sx, cY - sy * 0.3, cX + sx + sx * 0.3, cY)
    surface.SetDrawColor(240, 200, 0, 100)
    if turnmode == 1 then surface.SetDrawColor(240, 200, 0, 255) end
...
local function PaintSeatSwitcher(ent, pSeats, SeatCount)
    ...
    for _, v in pairs(simfphys.pSwitchKeysInv) do
        if input.IsKeyDown(v) then me.SwitcherTime = Time + 2 end
    end
