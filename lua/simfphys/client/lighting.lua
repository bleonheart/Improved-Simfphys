    ...
    for i, ent in pairs(vtable) do
        if IsValid(ent) then
            ...
            for i, proj in pairs(entTable.Projtexts) do
                ...
                if entTable.HasSpecialTurnSignals then if proj.trigger == 4 and (entTable.triggers[6] or entTable.triggers[7]) then trigger = false end end
...
local function SetupProjectedTextures(ent, vehiclelist)
    ...
    ent:CallOnRemove("remove_projected_textures", function(vehicle)
        for i, proj in pairs(ent.Projtexts) do
            ...
            if IsValid(thelamp) then thelamp:Remove() end
        end
    end)
end
            ...
            if not ent.LightsEMS[i].Damaged then
                ...
                if numcolors > 1 then
                    if ent.LightsEMS[i].Timer < Time then
                        ...
                        if ent.LightsEMS[i].Index > numcolors then ent.LightsEMS[i].Index = 1 end
                    end
                end

                local col = ent.LightsEMS[i].Colors[ent.LightsEMS[i].Index]
                if ent.LightsEMS[i].OnBodyGroups then Visible = ent:BodyGroupIsValid(ent.LightsEMS[i].OnBodyGroups) and Visible or 0 end
...
hook.Add("PostDrawTranslucentRenderables", "simfphys_draw_sprites", function(_, skybox, skybox3d)
    ...
    for i, ent in pairs(vtable) do
        if IsValid(ent) and not ent:IsDormant() then
            local entTable = ent:GetTable()
            if ent:GetEMSEnabled() then DrawEMSLights(ent) end
            ...
            for i, sprite in pairs(entTable.Sprites) do
                if not sprite.Damaged then
                    ...
                    if regTrigger or typeSpecial then
                        ...
                        if sprite.bodygroups then visible = BodyGroupIsValid(sprite.bodygroups, ent) and visible or 0 end
                        if visible and visible >= 0.6 then
                            visible = (visible - 0.6) / 0.4
                            render.SetMaterial(s_mat)
                            local c_Alpha = s_col["a"] * visible
                            if sprite.trigger == 6 or sprite.trigger == 7 or typeSpecial then c_Alpha = c_Alpha * (ent:GetFlasher() ^ 7) end
        ...
        for i, sprite in pairs(veh.Sprites) do
            if not sprite.Damaged then
                ...
                if Dist < Rad then
                    ...
                    if sprite.trigger >= 6 then veh.turnsignals_damaged = true end
    ...
    if istable(veh.Projtexts) then
        for i, proj in pairs(veh.Projtexts) do
            if not proj.Damaged then
                ...
                if Dist < Rad * 2 then veh.Projtexts[i].Damaged = true end
            end
        end
    end
