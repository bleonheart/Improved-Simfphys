...
if not simfphys.Fuel then
    ...
    hook.Add("PreRegisterSENT", "simfphys_RemoveFuelSents", function(_, class) if simfphysFuelSents[class] then return false end end)
    hook.Add("PreRegisterSWEP", "simfphys_RemoveFuelSents", function(_, class) if simfphysFuelSents[class] then return false end end)
end
...
if SERVER then
    ...
    function simfphys.BuildVehicleInfo(ent)
        ...
        for _, Entity in pairs(constraint.GetAllConstrainedEntities(ent)) do
            ...
            if IsValid(EPOBJ) then Mass = Mass + EPOBJ:GetMass() end
        end
    ...
    function simfphys.SpawnVehicle(Player, Pos, Ang, Model, Class, VName, VTable, bNoOwner)
        if not bNoOwner then if not gamemode.Call("PlayerSpawnVehicle", Player, Model, VName, VTable) then return end end
        ...
        if VTable.Members then
            ...
            if Ent.ModelInfo then
                ...
                if Ent.ModelInfo.Skin then Ent:SetSkin(Ent.ModelInfo.Skin) end
            ...
            if not simfphys.WeaponSystemRegister then
                if simfphys.ManagedVehicles then print("[SIMFPHYS ARMED] IS OUT OF DATE") end
            else
                timer.Simple(0.2, function() simfphys.WeaponSystemRegister(Ent) end)
                if (simfphys.armedAutoRegister and not simfphys.armedAutoRegister()) or simfphys.RegisterEquipment then
                    ...
                    simfphys.RegisterCamera = function(ent, offset_firstperson, offset_thirdperson, bLocalAng, attachment) simfphys.CameraRegister(ent, offset_firstperson, offset_thirdperson, bLocalAng, attachment) end
    ...
    function simfphys.SetOwner(ply, entity)
        ...
        if CPPI then
            ...
            if IsValid(ply) then entity:CPPISetOwner(ply) end
        end
    end
end
