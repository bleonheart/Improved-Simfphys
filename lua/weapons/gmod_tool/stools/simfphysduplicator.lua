...
if SERVER then
    ...
    local recv_interval_start = 0
    local recv_interval_len = 1
    local recv_interval_count = 0
    local recv_max = 500
    local recv_stop = 0
    local recv_delay_after_stop = 3600
    net.Receive("sphys_dupe", function(length, ply)
        if recv_stop == 1 then return end
        if recv_interval_start + recv_interval_len < os.time() then
            recv_interval_start = os.time()
            recv_interval_count = 0
        end

        recv_interval_count = recv_interval_count + 1
        if recv_interval_count > recv_max then
            recv_stop = 1
            timer.Simple(recv_delay_after_stop, function() recv_stop = 0 end)
...
if CLIENT then
    ...
    local TOOLMemory = {}
    net.Receive("sphys_dupe", function(length) TOOLMemory = net.ReadTable() end)
    local function GetSaves(panel)
        ...
        for k, v in pairs(saved_vehicles) do
            local printname = v
            if not selecteditem then selecteditem = printname end
            ...
            Button.DoClick = function(self) selecteditem = self.printname end
            index = index + 1
    ...
    function TOOL.BuildCPanel(panel)
        if not file.Exists("saved_vehicles", "DATA") then file.CreateDir("saved_vehicles") end
        ...
        Button:SetSize(280, 20)
        Button.DoClick = function(self)
            if isstring(selecteditem) then file.Delete("saved_vehicles/" .. selecteditem) end
            ScrollPanel:Clear()
            selecteditem = nil
            GetSaves(ScrollPanel)
        end
...
local function ValidateModel(model)
    ...
    for listname, _ in pairs(v_list) do
        if v_list[listname].Members.CustomWheels then
            ...
            if FrontWheel then FrontWheel = string.lower(FrontWheel) end
            if RearWheel then RearWheel = string.lower(RearWheel) end
...
function TOOL:GetVehicleData(ent, ply)
    ...
    if ent.WheelTool_Foffset then ply.TOOLMemory.WheelTool_Foffset = ent.WheelTool_Foffset end
    if ent.WheelTool_Roffset then ply.TOOLMemory.WheelTool_Roffset = ent.WheelTool_Roffset end
    if ent.snd_blowoff then ply.TOOLMemory.snd_blowoff = ent.snd_blowoff end
    if ent.snd_spool then ply.TOOLMemory.snd_spool = ent.snd_spool end
    if ent.snd_bloweron then ply.TOOLMemory.snd_bloweron = ent.snd_bloweron end
    if ent.snd_bloweroff then ply.TOOLMemory.snd_bloweroff = ent.snd_bloweroff end
            ...
            timer.Simple(0.5, function()
                ...
                for i = 1, table.Count(freezeWheels) do
                    ...
                    if IsValid(Wheel) then
                        ...
                        if IsValid(wPObj) then wPObj:EnableMotion(freezeWheels[i].dofreeze) end
        ...
        if Ent.CustomWheels then
            if Ent.GhostWheels then
                timer.Simple(Update and 0.25 or 0, function()
                    if not IsValid(Ent) then return end
                    if ply.TOOLMemory.WheelTool_Foffset and ply.TOOLMemory.WheelTool_Roffset then SetWheelOffset(Ent, ply.TOOLMemory.WheelTool_Foffset, ply.TOOLMemory.WheelTool_Roffset) end
