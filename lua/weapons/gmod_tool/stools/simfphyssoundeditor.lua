...
local function TextEntry(parent, name, concommand, ypos)
    ...
    TextEntry.OnValueChange = function(self, value) RunConsoleCommand(concommand, tostring(value)) end
end

local refresh = false
cvars.AddChangeCallback("simfphyssoundeditor_Idle", function(convar, oldValue, newValue) if oldValue ~= newValue then refresh = true end end)
cvars.AddChangeCallback("simfphyssoundeditor_RevDown", function(convar, oldValue, newValue) if oldValue ~= newValue then refresh = true end end)
cvars.AddChangeCallback("simfphyssoundeditor_ShiftUp", function(convar, oldValue, newValue) if oldValue ~= newValue then refresh = true end end)
...
function TOOL.BuildCPanel(panel)
    ...
    CheckBox.OnChange = function(self, checked) BuildMenu(Frame, panel) end
end
