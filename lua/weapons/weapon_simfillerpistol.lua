...
function SWEP:Initialize()
    ...
    local Owner = self:GetOwner()
    if IsValid(Owner) then Owner.usedFuel = 0 end
end
...
function SWEP:Think()
    if CLIENT then
        ...
        if FirstPerson then Pos = Owner:GetShootPos() + Owner:EyeAngles():Forward() * 10 + Owner:EyeAngles():Right() * 3 - Owner:EyeAngles():Up() * 2 end
        ...
        if particle then
            ...
            if FirstPerson then dir = Owner:EyeAngles() + Angle(-15, 10, 0) end
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if self:GetFuelType() ~= FUELTYPE_ELECTRIC then Owner.usedFuel = Owner.usedFuel + self.RefilAmount end
    end
