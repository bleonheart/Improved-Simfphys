        ...
        hook.Add("PreDrawHalos", "simfphys_remote_halos", function()
            ...
            if IsValid(weapon) then
                ...
                if not weapon:GetActive() then
                    ...
                    if IsValid(car) then halo.Add({car}, Color(0, 127, 255)) end
                end
            end
        end)
    end
...
function SWEP:Disable()
    ...
    if IsValid(car) then car.RemoteDriver = nil end
end
...
hook.Add("simfphysOnDelete", "simfphysOnDelete_remote", function(ent)
    ...
    local weapon = ply:GetActiveWeapon()
    if IsValid(weapon) and weapon:GetClass() == "weapon_simremote" then weapon:Disable() end
end)
...
function SWEP:Holster()
    if IsValid(self:GetCar()) then self:Disable() end
    return true
end
