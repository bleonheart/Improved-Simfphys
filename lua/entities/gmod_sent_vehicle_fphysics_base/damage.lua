function ENT:ApplyDamage(damage, type)
    if type == DMG_BLAST then damage = damage * 10 end
    if type == DMG_BULLET then damage = damage * 2 end
    ...
    if MaxHealth > 30 and NewHealth <= 31 and self:EngineActive() then self:DamagedStall() end
    if NewHealth <= 0 then
        ...
        if self:EngineActive() then self:DamagedStall() end
        self:SetCurHealth(0)
        return
    end
...
local function Spark(pos, normal, snd)
    ...
    if snd then sound.Play(Sound(snd), pos, 75) end
end
    ...
    if data.Speed > 60 and data.DeltaTime > 0.2 then
        ...
        if data.Speed > 1000 then
            ...
            if data.Speed > 250 then
                ...
                if not hitent then if simfphys.DamageMul > 1 then self:TakeDamage((data.Speed / 28) * simfphys.DamageMul, Entity(0), Entity(0)) end end
            end
