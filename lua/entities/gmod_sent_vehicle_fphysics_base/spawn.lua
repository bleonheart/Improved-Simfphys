...
function ENT:SetupEnteringAnims()
    ...
    for _, i in pairs(attachments) do
        ...
        if string.match(curstring, "exit", 1) then table.insert(self.Exitpoints, curstring) end
        if string.match(curstring, "enter", 1) then table.insert(self.Enterpoints, curstring) end
    end

    if table.Count(self.Enterpoints) < 1 then self.Enterpoints = nil end
    if table.Count(self.Exitpoints) < 1 then self.Exitpoints = nil end
end
...
function ENT:InitializeVehicle()
    ...
    if self.LightsTable then
        ...
        if vehiclelist then
            if vehiclelist.PoseParameters then self.LightsPP = vehiclelist.PoseParameters end
            if vehiclelist.BodyGroups then self:SetBodygroup(vehiclelist.BodyGroups.Off[1], vehiclelist.BodyGroups.Off[2]) end
        end
    end

    physObj:SetDragCoefficient(self.AirFriction or -250)
    physObj:SetMass(self.Mass * 0.75)
    if self.Inertia then physObj:SetInertia(self.Inertia) end
    ...
    if self.Attachments then
        for i = 1, table.Count(self.Attachments) do
            ...
            if self.Attachments[i].skin then prop:SetSkin(self.Attachments[i].skin) end
...
function ENT:SetValues()
    if istable(WireLib) then self:createWireIO() end
    self:SetGear(2)
    ...
    if self.CustomWheels then
        if self.CustomWheelModel then
            if not file.Exists(self.CustomWheelModel, "GAME") then
                if IsValid(self.EntityOwner) then self.EntityOwner:PrintMessage(HUD_PRINTTALK, "ERROR: \"" .. self.CustomWheelModel .. "\" does not exist! Removing vehicle. (Class: " .. self:GetSpawn_List() .. ")") end
                self:Remove()
                return
            end
            ...
            if self.SteerFront ~= false then
                ...
                if IsValid(pobj) then
                    ...
                    if IsValid(self.EntityOwner) then self.EntityOwner:PrintMessage(HUD_PRINTTALK, "ERROR: \"" .. self.CustomWheelModel .. "\" doesn't have an collision model! Removing vehicle. (Class: " .. self:GetSpawn_List() .. ")") end
            ...
            if self.CustomWheelPosML then self:CreateWheel(5, WheelML, self:LocalToWorld(self.CustomWheelPosML), self.RearHeight, self.RearWheelRadius, false, self:LocalToWorld(self.CustomWheelPosML + Vector(0, 0, self.CustomSuspensionTravel * 0.5)), self.CustomSuspensionTravel, self.RearConstant, self.RearDamping, self.RearRelativeDamping) end
            if self.CustomWheelPosMR then self:CreateWheel(6, WheelMR, self:LocalToWorld(self.CustomWheelPosMR), self.RearHeight, self.RearWheelRadius, true, self:LocalToWorld(self.CustomWheelPosMR + Vector(0, 0, self.CustomSuspensionTravel * 0.5)), self.CustomSuspensionTravel, self.RearConstant, self.RearDamping, self.RearRelativeDamping) end
        else
            if IsValid(self.EntityOwner) then self.EntityOwner:PrintMessage(HUD_PRINTTALK, "ERROR: no wheel model defined. Removing vehicle. (Class: " .. self:GetSpawn_List() .. ")") end
            self:Remove()
        end
    else
...
function ENT:CreateWheel(index, name, attachmentpos, height, radius, swap_y, poseposition, suspensiontravel, constant, damping, rdamping)
    ...
    if self.FrontWheelMass and (index == 1 or index == 2) then WheelMass = self.FrontWheelMass end
    if self.RearWheelMass and (index == 3 or index == 4 or index == 5 or index == 6) then WheelMass = self.RearWheelMass end
    ...
    if self.CustomWheels then
        ...
        if self.ModelInfo then if self.ModelInfo.WheelColor then self.GhostWheels[index]:SetColor(self.ModelInfo.WheelColor) end end
    ...
    if self.CustomWheels then
        if index == 1 or index == 2 then targetentity = self.SteerMaster or self end
        if index == 3 or index == 4 then targetentity = self.SteerMaster2 or self end
    end
