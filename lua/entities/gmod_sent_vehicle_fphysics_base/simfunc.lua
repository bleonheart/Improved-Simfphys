...
function ENT:SimulateEngine(IdleRPM, LimitRPM, Powerbandstart, Powerbandend, c_time)
    ...
    if not self:IsDriveWheelsOnGround() then selfTbl.Clutch = 1 end
    ...
    if self:GetCurHealth() <= self:GetMaxHealth() * 0.3 then MaxTorque = MaxTorque * (self:GetCurHealth() / (self:GetMaxHealth() * 0.3)) end
    ...
    if simfphys.Fuel then
        ...
        if Fuel <= 0 and IsRunning then self:StopEngine() end
    else
        self:SetFuelUse(-1)
    end
...
function ENT:SimulateTransmission(k_throttle, k_brake, k_fullthrottle, k_clutch, k_handbrake, k_gearup, k_geardown, isauto, IdleRPM, Powerbandstart, Powerbandend, shiftmode, cruisecontrol, curtime)
    ...
    if isnumber(self.ForceTransmission) then isauto = self.ForceTransmission <= 1 end
    if not isauto then
        ...
        if k_gearup ~= self.GearUpPressed then
            ...
            if k_gearup == 1 then
                if self.CurrentGear ~= GearsCount then self.ThrottleDelay = curtime + 0.4 - 0.4 * k_clutch end
        ...
        if k_geardown ~= self.GearDownPressed then
            ...
            if k_geardown == 1 then
                self.CurrentGear = math.Clamp(self.CurrentGear - 1, 1, GearsCount)
                if self.CurrentGear == 1 then self.ThrottleDelay = curtime + 0.25 end
            end
        end
    else
        ...
        if self:IsDriveWheelsOnGround() then
            ...
            elseif self.ForwardSpeed >= -350 then
                if Throttle > 0 then self.Brake = 0 end
                self.CurrentGear = 1
            end
...
function ENT:SimulateWheels(k_clutch, LimitRPM)
    ...
    for i = 1, table.Count(selfTbl.Wheels) do
        ...
        if IsValid(Wheel) then
            ...
            if WheelPos.IsFrontWheel then
                if FrontWheelCanTurn then selfTbl.VehicleData["spin_" .. i] = selfTbl.VehicleData["spin_" .. i] + TurnWheel end
            else
                if RearWheelCanTurn then selfTbl.VehicleData["spin_" .. i] = selfTbl.VehicleData["spin_" .. i] + TurnWheel end
            end
            ...
            if not selfTbl.PhysicsEnabled then
                ...
                if IsValid(phys) then phys:ApplyForceCenter(Force * 185 * OnGround) end
            end
        end
    end
