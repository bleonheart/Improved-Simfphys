...
function ENT:GetPassengerSeats()
    if not istable(self.pSeat) then
        ...
        for _, v in pairs(self:GetChildren()) do
            if v ~= DriverSeat and v:GetClass():lower() == "prop_vehicle_prisoner_pod" then table.insert(self.pSeat, v) end
        end
    end
    return self.pSeat
end
...
function ENT:GetSeatAnimation(ply)
    ...
    if not istable(pSeatTBL) then return -1 end
    local seq = pSeatTBL.anim
    if not isstring(seq) then return -1 end
