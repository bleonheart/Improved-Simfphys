...
function ENT:CalcFlasher()
    ...
    if self.Flasher >= 1 then self.Flasher = self.Flasher - 1 end
...
function ENT:DamageEffects(selfTable)
    ...
    if selfTable.wasSmoke ~= smoke then
        ...
        if smoke then
            ...
            if selfTable.smokesnd then selfTable.smokesnd:Stop() end
        end
    end
    ...
    if selfTable.wasFire ~= fire then
        ...
        if fire then
            ...
            if selfTable.firesnd then selfTable.firesnd:Stop() end
        end
    end
...
function ENT:ManageSounds(Active, fThrottle, LimitRPM)
    ...
    if Throttle ~= selfTable.OldThrottle2 then
        ...
        if Throttle == 0 then if selfTable.SmoothRPM > LimitRPM * 0.6 then self:Backfire() end end
    end
    ...
    if Active ~= selfTable.OldActive then
        ...
        if Active then
            ...
            if Health <= (MaxHealth * 0.6) then selfTable.DamageSnd:PlayEx(0, 0) end
                ...
                if selfTable.FadeThrottle == 0 and Clutch == 0 then
                    if selfTable.SmoothRPM >= FadeRPM then
                        if IsCruise ~= true then
                            if selfTable.LowRPM then selfTable.LowRPM:Stop() end
                ...
                if selfTable.SmoothRPM >= FadeRPM and Gear > 3 then
                    if Clutch ~= 1 then
                        if selfTable.OldGear < Gear then
                            if selfTable.HighRPM then selfTable.HighRPM:Stop() end
...
function ENT:SetSoundPreset(index)
    ...
    if vehiclelist then if not self.ExhaustPositions then self.ExhaustPositions = vehiclelist.Members.ExhaustPositions end end
    if index == -1 then
        if vehiclelist then
...
function ENT:SaveStopSounds()
    if self.HighRPM then self.HighRPM:Stop() end
    if self.LowRPM then self.LowRPM:Stop() end
    if self.Idle then self.Idle:Stop() end
    if self.Valves then self.Valves:Stop() end
    if self.DamageSnd then self.DamageSnd:Stop() end
end

function ENT:OnRemove()
    self:SaveStopSounds()
    if self.smokesnd then self.smokesnd:Stop() end
    if self.firesnd then self.firesnd:Stop() end
end
