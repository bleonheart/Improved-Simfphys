...
function ENT:ApplyDupeInfo(ply, ent, info, GetEntByID)
    if istable(WireLib) then WireLib.ApplyDupeInfo(ply, ent, info, GetEntByID) end
end

function ENT:PreEntityCopy()
    if istable(WireLib) then duplicator.StoreEntityModifier(self, "WireDupeInfo", WireLib.BuildDupeInfo(self)) end
end

function ENT:PostEntityPaste(Player, Ent, CreatedEntities)
    if istable(WireLib) then if Ent.EntityMods and Ent.EntityMods.WireDupeInfo then WireLib.ApplyDupeInfo(Player, Ent, Ent.EntityMods.WireDupeInfo, EntityLookup(CreatedEntities)) end end
end
...
function ENT:Think()
    ...
    if selfTable.NextTick < Time then
        ...
        if IsValid(selfTable.DriverSeat) then
            ...
            Driver = IsValid(selfTable.RemoteDriver) and selfTable.RemoteDriver or Driver
            local OldDriver = self:GetDriver()
            if OldDriver ~= Driver then
                if self:GetIsVehicleLocked() then self:UnLock() end
                ...
                if HasDriver then
                    ...
                    if Driver:GetInfoNum("cl_simfphys_autostart", 1) > 0 then self:StartEngine() end
                else
                    if selfTable.ems then selfTable.ems:Stop() end
                    if selfTable.horn then selfTable.horn:Stop() end
                    if selfTable.PressedKeys then
                        for k, v in pairs(selfTable.PressedKeys) do
                            if isbool(v) then selfTable.PressedKeys[k] = false end
                        end
                    end

                    if selfTable.keys then
                        for i = 1, table.Count(selfTable.keys) do
                    ...
                    if HadDriver then
                        if OldDriver:GetInfoNum("cl_simfphys_autostart", 1) > 0 then
                            ...
                            self:ResetJoystick()
                            if not self:EngineActive() then self:SetActive(false) end
                        end
                    else
                        self:SetActive(false)
                        self:StopEngine()
        ...
        if self:IsInitialized() then
            ...
            self:ControlHorn()
            if istable(WireLib) then self:UpdateWireOutputs() end
...
function ENT:TriggerInput(name, value)
    ...
    if name == "Steer" then
        ...
        for i = 1, table.Count(self.Wheels) do
            ...
            if IsValid(Wheel) then Wheel:PhysWake() end
        end
    end

    if name == "Throttle" then self.PressedKeys["joystick_throttle"] = math.Clamp(value, 0, 1) end
    if name == "Brake/Reverse" then self.PressedKeys["joystick_brake"] = math.Clamp(value, 0, 1) end
    ...
    if name == "Set Gear" then self:ForceGear(math.Round(value, 0)) end
    if name == "Clutch" then self.PressedKeys["joystick_clutch"] = math.Clamp(value, 0, 1) end
    if name == "Handbrake" then self.PressedKeys["joystick_handbrake"] = (value > 0) and 1 or 0 end
end
...
function ENT:OnActiveChanged(name, old, new)
    ...
    if new == true then
        ...
        if self:GetEMSEnabled() then if self.ems then self.ems:Play() end end
        ...
        self:StopEngine()
        if TurboCharged then self.Turbo:Stop() end
        if SuperCharged then
            self.Blower:Stop()
    ...
    if istable(self.Wheels) then
        for i = 1, table.Count(self.Wheels) do
            local Wheel = self.Wheels[i]
            if IsValid(Wheel) then Wheel:SetOnGround(0) end
        end
    end
end
...
function ENT:OnThrottleChanged(name, old, new)
    ...
    if new == 1 then if Health < MaxHealth * 0.6 then if Active then if math.Round(math.random(0, 4), 0) == 1 then self:DamagedStall() end end end end
    if new == 0 then
...
function ENT:WaterPhysics()
    ...
    if not self.IsInWater then
        if self:EngineActive() then self:EmitSound("vehicles/jetski/jetski_off.wav") end
        self.IsInWater = true
        self.EngineIsOn = 0
        self.EngineRPM = 0
...
function ENT:ControlLighting(curtime)
    if (self.NextLightCheck or 0) < curtime and self.LightsActivated ~= self.DoCheck then
        ...
        if self.LightsActivated then self:SetLightsEnabled(true) end
    end
end
...
function ENT:SimulateVehicle(curtime)
    ...
    if Active then
        ...
        if cruise then
            if (selfTable.PressedKeys["joystick_gearup"] + selfTable.PressedKeys["joystick_geardown"] + selfTable.PressedKeys["joystick_handbrake"] + selfTable.PressedKeys["joystick_throttle"] + selfTable.PressedKeys["joystick_clutch"] + selfTable.PressedKeys["joystick_brake"]) > 0 then self:SetIsCruiseModeOn(false) end
            if k_Shift then selfTable.cc_speed = math.Round(self:GetVelocity():Length(), 0) + 70 end
            if Alt == 1 then selfTable.cc_speed = math.Round(self:GetVelocity():Length(), 0) - 25 end
        end
    ...
    if selfTable.CustomWheels then self:PhysicalSteer() end
end
...
function ENT:PhysicalSteer()
    local selfTbl = self:GetTable()
    if IsValid(selfTbl.SteerMaster) then
...
function ENT:CanStart()
    ...
    if simfphys.Fuel then FuelSystemOK = self:GetFuel() > 0 end
...
function ENT:StallAndRestart(nTimer, bIgnoreSettings)
    ...
    self:StopEngine()
    local ply = self:GetDriver()
    if IsValid(ply) and not bIgnoreSettings then if ply:GetInfoNum("cl_simfphys_autostart", 1) <= 0 then return end end
...
function ENT:GetClosestSeat(ply)
    ...
    for i = 1, table.Count(self.pSeat) do
        ...
        if Dist < Distance then Seat = self.pSeat[i] end
    end
    return Seat
end
...
function ENT:HasPassengerEnemyTeam(ply)
    ...
    if IsValid(self:GetDriver()) then if self:GetDriver():Team() ~= myteam then return true end end
    if self.PassengerSeats then
        for i = 1, table.Count(self.pSeat) do
            if IsValid(self.pSeat[i]) then
                local Passenger = self.pSeat[i]:GetDriver()
                if IsValid(Passenger) then if Passenger:Team() ~= myteam then return true end end
            end
        end
    end
    return false
...
function ENT:SetPhysics(enable)
    if enable then
        if not self.PhysicsEnabled then
            for i = 1, table.Count(self.Wheels) do
                local Wheel = self.Wheels[i]
                if IsValid(Wheel) then Wheel:GetPhysicsObject():SetMaterial("jeeptire") end
            end
...
function ENT:SetSuspension(index, bIsDamaged)
    ...
    if IsValid(Elastic) then Elastic:Fire("SetSpringLength", newheight) end
    if self.StrengthenSuspension == true then
        local Elastic2 = self.Elastics[index * 10]
        if IsValid(Elastic2) then Elastic2:Fire("SetSpringLength", newheight) end
    end
end
...
function ENT:OnFrontSuspensionHeightChanged(name, old, new)
    ...
    if IsValid(self.Wheels[1]) then
        ...
        if IsValid(Elastic) then Elastic:Fire("SetSpringLength", self.FrontHeight + self.VehicleData.suspensiontravel_fl * -new) end
        if self.StrengthenSuspension == true then
            local Elastic2 = self.Elastics[10]
            if IsValid(Elastic2) then Elastic2:Fire("SetSpringLength", self.FrontHeight + self.VehicleData.suspensiontravel_fl * -new) end
        end
    end

    if IsValid(self.Wheels[2]) then
        local Elastic = self.Elastics[2]
        if IsValid(Elastic) then Elastic:Fire("SetSpringLength", self.FrontHeight + self.VehicleData.suspensiontravel_fr * -new) end
        if self.StrengthenSuspension == true then
            local Elastic2 = self.Elastics[20]
            if IsValid(Elastic2) then Elastic2:Fire("SetSpringLength", self.FrontHeight + self.VehicleData.suspensiontravel_fr * -new) end
        end
    end
end
...
function ENT:OnRearSuspensionHeightChanged(name, old, new)
    ...
    if IsValid(self.Wheels[3]) then
        ...
        if IsValid(Elastic) then Elastic:Fire("SetSpringLength", self.RearHeight + self.VehicleData.suspensiontravel_rl * -new) end
        if self.StrengthenSuspension == true then
            local Elastic2 = self.Elastics[30]
            if IsValid(Elastic2) then Elastic2:Fire("SetSpringLength", self.RearHeight + self.VehicleData.suspensiontravel_rl * -new) end
        end
    end

    if IsValid(self.Wheels[4]) then
        local Elastic = self.Elastics[4]
        if IsValid(Elastic) then Elastic:Fire("SetSpringLength", self.RearHeight + self.VehicleData.suspensiontravel_rr * -new) end
        if self.StrengthenSuspension == true then
            local Elastic2 = self.Elastics[40]
            if IsValid(Elastic2) then Elastic2:Fire("SetSpringLength", self.RearHeight + self.VehicleData.suspensiontravel_rr * -new) end
        end
    end
...
function ENT:OnTurboCharged(name, old, new)
    ...
    elseif new == false then
        if self.Turbo then self.Turbo:Stop() end
    end
end
...
function ENT:OnSuperCharged(name, old, new)
    ...
    if new == true and Active then
        ...
        self.BlowerWhine:PlayEx(0, 0)
    elseif new == false then
        if self.Blower then self.Blower:Stop() end
        if self.BlowerWhine then self.BlowerWhine:Stop() end
    end
end
...
function ENT:OnRemove()
    ...
    if self.Blower then self.Blower:Stop() end
    if self.BlowerWhine then self.BlowerWhine:Stop() end
    if self.horn then self.horn:Stop() end
    if self.ems then self.ems:Stop() end
    self:OnDelete()
...
function ENT:SetOnFire(bOn)
    ...
    if bOn then self:DamagedStall() end
end
