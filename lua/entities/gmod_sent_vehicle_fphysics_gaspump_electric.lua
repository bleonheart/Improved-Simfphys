...
function ENT:SetupDataTables()
    ...
    if SERVER then self:NetworkVarNotify("Active", self.OnActiveChanged) end
end
...
function ENT:Use(ply)
    if not self:GetActive() then
        if not ply.gas_InUse then
            ...
            if IsValid(weapon) and weapon:GetClass() == "weapon_simfillerpistol" then weapon:SetFuelType(FUELTYPE_ELECTRIC) end
        end
    else
...
function ENT:OnActiveChanged(name, old, new)
    ...
    if new then
        ...
        if IsValid(self.PumpEnt) then self.PumpEnt:SetNoDraw(true) end
    else
        if IsValid(self.PumpEnt) then self.PumpEnt:SetNoDraw(false) end
        if self.sound then
    ...
    if IsValid(ply) then
        ...
        if ply:Alive() then
            if ply:InVehicle() then
                if ply:HasWeapon("weapon_simfillerpistol") then ply:StripWeapon("weapon_simfillerpistol") end
                ply.gas_InUse = false
                self:Disable()
            else
...
function ENT:OnRemove()
    ...
    if IsValid(ply) then
        ply.gas_InUse = false
        if ply:Alive() then if ply:HasWeapon("weapon_simfillerpistol") then ply:StripWeapon("weapon_simfillerpistol") end end
    end
end
